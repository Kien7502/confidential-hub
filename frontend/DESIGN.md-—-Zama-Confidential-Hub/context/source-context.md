...
Canonical design-system title: Zama cToken App Design System
...
- https://github.com/Kien7502/zama-cToken-shield
- https://zama-ctoken-shield.vercel.app
...
1. For each linked repository, run the bounded intake command before writing design-system files. The command tries this-device access first (`git clone`, then authenticated GitHub CLI via `gh auth login --web`) and uses the Composio GitHub connector only as a connector-platform fallback.
   - `"$OD_NODE_BIN" "$OD_BIN" tools connectors github-design-context --repo 'https://github.com/Kien7502/zama-cToken-shield' --output context/github/Kien7502-zama-cToken-shield.md`
...
- C:\Users\PC\Desktop\claude\zama-cToken\frontend
...
1. For each linked folder, run the bounded local intake command before writing design-system files:
   - `"$OD_NODE_BIN" "$OD_BIN" tools connectors local-design-context --path 'C:\Users\PC\Desktop\claude\zama-cToken\frontend' --output context/local-code/frontend.md`
