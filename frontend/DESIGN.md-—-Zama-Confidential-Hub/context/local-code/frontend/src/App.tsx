      ...
      <section className="asset-detail-modal" onClick={(event) => event.stopPropagation()}>
        ...
        {isEncrypted ? (
          <div className="encrypted-notice">
            <div className="encrypted-notice-head">
              <Lock size={16} />
            ...
            <button className="primary encrypted-decrypt" disabled={actionLocked || decryptPhase !== "idle"} onClick={() => void decryptBalance()}>
              <Eye size={16} />
              {decryptPhase === "signing" ? "SIGNING" : decryptPhase === "decrypting" ? "DECRYPTING" : "DECRYPT"}
            </button>
...
function TokenDropdown({
  ...
  return (
    <div className="token-dropdown" onClick={(event) => event.stopPropagation()}>
      <button type="button" className="token-dropdown-trigger" disabled={options.length === 0} onClick={() => setOpen((current) => !current)}>
        {selected ? (
          <>
        ...
        <ChevronDown size={16} className="token-dropdown-caret" />
      </button>
        ...
        <div className="token-dropdown-menu">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
              className={option.value === selected?.value ? "token-dropdown-item active" : "token-dropdown-item"}
              onClick={() => {
  ...
  return (
    <section className="send-page">
      ...
      <FlowCard
        ...
        selector={
          <TokenDropdown
            options={tokens.map((token) => ({ value: token.address, token: token.metadata, confidential: true, underlying: token.underlying }))}
...
function FaucetPage({
  ...
  return (
    <ActionPanel icon={Banknote} locked={locked} title="Claim mock ERC20" disabled={!pair || !allowed || busy} submitLabel={busy ? "Claiming" : "Claim"} onSubmit={submit}>
      <div className="faucet-token-grid">
